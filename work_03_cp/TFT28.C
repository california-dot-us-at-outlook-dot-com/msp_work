/********************************************************************
文件功能：320x240TFT驱动程序，控制TFT实现汉字,字符显示,画点功能。
********************************************************************/
...
/****************************************************************************************************
//	实现功能：	TFT指定范围填充，用于生成2D按钮
//	输入参数：	bColor 清屏所使用的背景色
****************************************************************************************************/
void LCD_Full(unsigned int x0,unsigned int x1,unsigned int y0,unsigned int y1,unsigned int bColor)
{
 unsigned int i,j;
 LCD_SetPos(x0,x1,y0,y1);													//320x240，坐标从0开始
 for (i=0;i<(x1-x0+1);i++)
	{
	   for (j=0;j<(y1-y0+1);j++)
	       Write_Data_U16(bColor);
...
****************************************************************************************************/
#include"GB2435.h"
void LCD_PutSingleChar2435(unsigned int x, unsigned int y, char c, unsigned int fColor, unsigned int bColor)
{
 unsigned int i,j;
	
 LCD_SetPos(x,x+24-1,y,y+35-1);                    //设置字符显示位置
 for(i=0; i<105;i++) {                             //循环写入16字节，一个字符为16字节
		unsigned char m=Font24x35[c*105+i];  		//提取c字符的第i个字节以,c减去0x20是由于Ascii码库中的0~1f被去掉
		for(j=0;j<8;j++) {                        		//循环写入8位，一个字节为8位
			if((m&0x80)==0x80) {                  			//判断最高位是否为1
				Write_Data_U16(fColor);           				//最高位为1，写入字符颜色
				}
